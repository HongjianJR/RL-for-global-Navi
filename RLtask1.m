...
clc
clear all
load ('task1.mat'); % reward 100*4
for m = 1:4
    fprintf('case %d\n', m);
    for gamma = [0.5, 0.9] % discount rate
        ...
        for run = 1:10 % 10 runs
            ...
            for trial = 1:t_trials % each run has max 3000 trials
                ...
                while k < 300 && alpha >= 0.005 % each trial max 300 moves 
                    ...
                    epsilon = alpha;
                    %%  epsilon greedy
                    temp = find(reward(s(k),:)~=-1); % idx for valid actions
                    ...
                    if x == 1   % exploit
                        ...
                        temp2 = find(reward(s(k),:)~=reward(s(k),temp(t)) & reward(s(k),:)~=-1); % idx of other options
                    ...
                    s(k+1) = move(s(k), a(k));
                    Q(s(k),a(k)) = Q(s(k),a(k))+alpha*(reward(s(k),a(k))+gamma*max(Q(s(k+1),:))-Q(s(k),a(k))); % update Q function
            ...
            [total_reward(run),p] = getreward(optimal_policy(:,run), reward); % calculate reward
...
function [total_reward,move_policy] = getreward(optimal_policy, reward)
n = 0;
state = 1;
move_policy = state;
total_reward = 0;
